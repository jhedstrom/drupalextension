...
class DrupalContext extends RawDrupalContext implements TranslatableContext {
   ...
   *
   * Set in the BeforeScenario hook so the BeforeStep hook does not have to
   * probe the driver again on every step.
   */
   ...
   *
   * Defaults to TRUE so scenarios where BigPipe handling is irrelevant (skip
   * tag, JS driver, opt-out config) do not pay any cost. The BeforeScenario
   * hook flips this to FALSE when the cookie should be kept alive across
   * user logins / cookie-clearing redirects.
   */
  ...
  /**
   * Sets the BigPipe NOJS cookie if the driver is started and supports it.
   *
   ...
   * 'http-equiv=refresh' fallback, so authenticated-user assertions
   * silently miss messages and blocks. Setting the 'big_pipe_nojs' cookie
   * tells Drupal to render the page fully server-side, restoring the
   * markup the test expects.
   *
   ...
   * this hook, a cookie set in BeforeScenario disappears the first time
   * the test user changes - and any subsequent authenticated-user
   * assertion fails the same way the original issue describes.
   */
  #[BeforeStep]
   ...
   *
   * Returns silently when the driver is not started yet (Selenium starts
   * lazily, and BrowserKit can be unstarted between subprocess runs),
   * when JavaScript is supported, when BigPipe is not bootstrapped, or
   * when the cookie is already present.
   */
  protected function bigPipeApplyCookie(): void {
    ...
    try {
      ...
      // Driver session is not ready - the next step's BeforeStep will retry.
    }
  }
   ...
   *
   * Defaults to TRUE so the fix is opt-out, not opt-in. Projects that need
   * to keep streaming behaviour for their own assertions can set
   ...
   * that case but we keep the no-op behaviour explicit so sites without
   ...
   * visit. An unstarted driver is treated as non-JS, and the per-step
   * BeforeStep hook re-applies the cookie once the driver is started.
   */
