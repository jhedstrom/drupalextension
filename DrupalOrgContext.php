...
class DrupalOrgContext extends BehatContext
{
  ...
  public function iAmAt($arg1)
  {
    ...
    $session->visit($this->base_url .'/project/modules');
  ...
  public function iSearchFor($arg1)
  {
	...
	$element->fillField('edit-text', $this->search_value);
  ...
  public function iShouldSeeAListOfModulesWhichMentionMasquerade()
  {
	$session = $this->mink->getSession();
	$element = $session->getPage();
	$result = $element->findLink($this->search_value);
	if (empty($result)) {
		throw new Exception("No link to $this->search_value on ". $session->getCurrentUrl());
	}
  }
}
